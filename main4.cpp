...
void MMultk1_Add01(long m, long n, double *a, double *b, double *c);             // c=c+a*b
void MMultk1_Add02(long m, long n, double *a, double *b, double *c, double *d);  // d=d+c+a*b
...
void LDSLearn(double *Y, double *A, double *C, double *Q, double *R, double *x_init, double *V_init, int T, int dimx, int dimy, int iter){
  ...
  for (int ite = 0; ite < iter; ite++) {
    ...
    #pragma omp parallel for num_threads(nrT)
    for(int t=0; t<T; t++){
      // #pragma omp task firstprivate(t) shared(delta, gamma, beta,Y,Xhat,Vhat_now,Vhat_pre)
      // {
        double* y=(double*) aligned_malloc(dimy * sizeof(double));
        double* xxhat=(double*) aligned_malloc(dimx * sizeof(double));//x^hat_t
        double* xhat_pre=(double*) aligned_malloc(dimx * sizeof(double));//x^hat_{t-1}
        double* vvhat_now=(double*) aligned_malloc(dimx*dimx * sizeof(double));//V^hat_t
        double* vhat_pre=(double*) aligned_malloc(dimx*dimx * sizeof(double));//V^hat_{t,t-1}
      // #pragma omp parallel num_threads(nrT)
      // {
      //get y_t
        ...
        // #pragma omp for //nowait
        for(int j=0; j<dimx; j++){
          xxhat[j]=Xhat[t*dimx+j];
        }
        //get V^hat_t
        // #pragma omp for 
        for(int j=0; j<dimx*dimx; j++){
          vvhat_now[j]=Vhat_now[t*dimx*dimx+j];
        }
      // }
        //update delta
        MMultk1_Add01(dimy, dimx, y, xxhat, delta);
        //update gamma
        MMultk1_Add02(dimx, dimx, xxhat, xxhat, vvhat_now, gamma);
        ...
        if(t>0){
          ...
          // }
          MMultk1_Add02(dimx, dimx, xxhat, xhat_pre, vhat_pre, beta);
        }
      // }
    // }
    // #pragma omp taskwait
...
void MMultk1_Add01(long m, long n, double *a, double *b, double *c) {
  for (long i = 0; i < m; i++) {
    for (long j = 0; j < n; j++) {
      double me = a[i] * b[j];
      // #pragma omp critical
...
void MMultk1_Add02(long m, long n, double *a, double *b, double *c, double *d) {
  for (long i = 0; i < m; i++) {
    for (long j = 0; j < n; j++) {
