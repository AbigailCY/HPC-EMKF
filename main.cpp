...
int main(int argc, char** argv) {
  ...
  init(Y, A, C, Q, R, x_init, V_init, T, dimx, dimy, 1);  
  ...
  writeVector("Xs_pred.txt", Xhat, T*dimx);
  aligned_free(Xhat);
...
void init(double *Y,double *A0,double *C0,double *Q0,double *R0,double *x_init,double *V_init, int L, int dimx, int dimy, int generate) {
  ...
  Identity0(dimx,dimy,C0,1);
  Identity0(dimy,dimy,R0,1);

  if (generate == 0) {
  
  for(int i=0; i<L*dimy;i++){
    Y[i]=1;
  }
  x_init[0]=0;
  x_init[1]=0;
  ...
  // // copy the initialized value (just for test)
