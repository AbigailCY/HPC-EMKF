...
int main(int argc, char** argv) {
  ...
  while ((c = getopt (argc, argv, "x:y:t:o:i:n:h")) != -1) {
      ...
      case 'o':
          ...
          break;
      case 'h':
          printf("-x5 -y2 -t500 -o0 -i1 -n4\n-x16 -y16 -t500 -o1 -i1 -n4\n-x50 -y50 -t500 -o2 -i1 -n4\n-x100 -y100 -t500 -o3 -i1 -n4\n-x150 -y150 -t200 -o3 -i1 -n4\n-x200 -y200 -t200 -o3 -i1 -n4\n-x250 -y250 -t500 -o3 -i1 -n4\n");
          exit(EXIT_SUCCESS);
          break;
      case 'i':
          iter = atoi(optarg);
...
void LDSInference(double *Y, double *A, double *C, double *Q, double *R, double *x_init, double *V_init, int T, int dimx, int dimy, double* Xhat, double* Vhat_now, double* Vhat_pre){
  ...
  for(int t=0; t<T; t++){
    ...
    Minverse1(dimy, help_yy_2, help_yy_1);
  ...
  for(int t=T-1; t>0; t--){
    ...
    //obtain J_{t-1}
    Minverse1(dimx, v_pre, help_xx_1);
...
void LDSLearn(double *Y, double *A, double *C, double *Q, double *R, double *x_init, double *V_init, int T, int dimx, int dimy, int iter){
  ...
  for(int t=0; t<T; t++){
    ...
    MMult1(dimy, dimy, 1, y, y, temp_yy_1);
    Madd1(dimy, dimy, alpha, temp_yy_1, alpha); //may have issue when parallelizing.
  // }
  }
  ...
  for (int ite = 0; ite < iter; ite++) {
    ...
    for(int t=0; t<T; t++){
      ...
      //update delta
      MMult1(dimy, dimx, 1, y, xhat, temp_yx_1);
      Madd1(dimy, dimx, delta, temp_yx_1, delta);//may have issue when parallelizing.
      //update gamma
      MMult1(dimx, dimx, 1, xhat, xhat, temp_xx_1);
      Madd1(dimx, dimx, gamma, temp_xx_1, temp_xx_2);
      Madd1(dimx, dimx, temp_xx_2, vhat_now, gamma);
      ...
      if(t>0){
        ...
        MMult1(dimx, dimx, 1, xhat, xhat_pre, temp_xx_1);
        Madd1(dimx, dimx, beta, temp_xx_1, temp_xx_2);
        Madd1(dimx, dimx, temp_xx_2, vhat_pre, beta);
      }
    // }
    }
    // printf("loop time: %8f\n", t.toc()); 
    ...
    //obtain gamma_1
    MMult1(dimx, dimx, 1, xhat, xhat, temp_xx_1);
    Msubstract1(dimx, dimx, gamma, temp_xx_1, temp_xx_2);
    Msubstract1(dimx, dimx, temp_xx_2, vhat_now, gamma1);
    ...
    MMult1(dimx, dimx, 1, xhat, xhat, temp_xx_1);
    Msubstract1(dimx, dimx, gamma, temp_xx_1, temp_xx_2);
    Msubstract1(dimx, dimx, temp_xx_2, vhat_now, gamma2);


    //M step
    //update C
    Minverse1(dimx, gamma, temp_xx_1);
    MMult1(dimy, dimx, dimx, delta, temp_xx_1, C);
    //update R
    Mtranspose1(dimy, dimx, delta, delta_transpose);
    MMult1(dimy, dimy, dimx, C, delta_transpose, temp_yy_1);
    Msubstract1(dimy, dimy, alpha, temp_yy_1, temp_yy_2);
    Mdivide1(dimy, dimy, temp_yy_2, R, T);
    //update A
    // t.tic();
    Minverse1(dimx, gamma1, temp_xx_1);
    // printf("inverse time: %8f\n", t.toc()); 
    MMult1(dimx, dimx, dimx, beta, temp_xx_1, A);
    //update Q
    Mtranspose1(dimx, dimx, beta, beta_transpose);

    // t.tic();
    MMult1(dimx, dimx, dimx, A, beta_transpose, temp_xx_1);
    // printf("multiple time: %8f\n", t.toc()); 

    // t.tic();
    Msubstract1(dimx, dimx, gamma2, temp_xx_1, temp_xx_2);
    // printf("add time: %8f\n", t.toc()); 
    Mdivide1(dimx, dimx, temp_xx_2, Q, T-1);
    //update x^0_1
    for(int j=0; j<dimx; j++){
      x_init[j]=Xhat[j];
    }
