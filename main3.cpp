  ...
  // }
    ...
    for(int t=0; t<T; t++){
      #pragma omp task firstprivate(t) shared(alpha,Y)
      {
        ...
        MMultk1_Add1(dimy, dimy, y, y, alpha);
      }
    }
    #pragma omp taskwait
    }
  ...
  for (int ite = 0; ite < iter; ite++) {
    ...
    for(int t=0; t<T; t++){
      ...
      //get y_t
        ...
        //update delta
        MMultk1_Add1(dimy, dimx, y, xxhat, delta);
        //update gamma
        MMultk1_Add2(dimx, dimx, xxhat, xxhat, vvhat_now, gamma);
        ...
        if(t>0){
          ...
          MMultk1_Add2(dimx, dimx, xxhat, xhat_pre, vhat_pre, beta);
        }
      }
    }
    // #pragma omp taskwait
